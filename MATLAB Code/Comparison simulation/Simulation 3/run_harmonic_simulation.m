function run_harmonic_simulation()
    ...
    % Injects 5th and 7th harmonic currents, typical of a rectifier load
    ...
    disp(['Running HARMONIC PERFORMANCE test for ' control_mode '...']);
    ...
    % Calculate THD for the phase A voltage in the last few cycles
    fs = 1 / (t(2)-t(1)); % Estimate sample frequency
    idx_start = find(t >= 0.3, 1);
    thd_val = thd(vC_a(idx_start:end), fs, 7); % Calculate THD up to 7th harmonic
...
function dxdt = harmonic_dynamics_abc(t, x, P, control_mode)
    ...
    theta = w_inst * t + delta; % Controller's internal angle
    
    % Park transform for measurements
    T_park = (2/3) * [cos(theta) cos(theta - 2*pi/3) cos(theta + 2*pi/3);
                      -sin(theta) -sin(theta - 2*pi/3) -sin(theta + 2*pi/3)];
    vC_dq = T_park * vC_abc;
    iL_dq = T_park * iL_abc;
    
    % Calculate filtered power (used by Droop & VSM)
    P_elec = vC_dq(1)* iL_dq(1) + vC_dq(2)*iL_dq(2);
    
    % --- Controller Dynamics ---
    % (This is the same unified controller model from the robustness test)
    P_ref = P.P_ref;
    switch control_mode
        case 'Droop'
        ...
        case 'VOC'
            ...
            target_delta = asin(p_filt * P.Lf * P.w_nom / P.V_peak^2);
            if isnan(target_delta), target_delta = delta; end
            dw_dev_dt = 20 * (target_delta - delta) - 2 * w_dev; 
    end
    ddelta_dt = w_dev;

    % Controller output is a desired voltage in its dq frame
    v_inv_ref_dq = [P.V_peak; 0];
    
    % Inverse Park transform to get abc voltage reference
    T_inv_park = [cos(theta)             -sin(theta);
    ...
    if t >= P.t_harmonic_start
        % Find angle of fundamental voltage to sync harmonic currents
        theta_fund = atan2(vC_abc(2) - vC_abc(3), sqrt(3)*vC_abc(1));
        i_harmonic_abc = [ P.I5_mag * sin(5 * w_inst * t + theta_fund);
                           P.I5_mag * sin(5 * (w_inst * t - 2*pi/3) + theta_fund);
                           P.I5_mag * sin(5 * (w_inst * t + 2*pi/3) + theta_fund) ];
        i_harmonic_abc = i_harmonic_abc + ...
                         [ P.I7_mag * sin(7 * w_inst * t + theta_fund);
                           P.I7_mag * sin(7 * (w_inst * t - 2*pi/3) + theta_fund);
                           P.I7_mag * sin(7 * (w_inst * t + 2*pi/3) + theta_fund) ];
    end
