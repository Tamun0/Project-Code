function run_harmonic_simulation_pure_droop()
    ...
    % This version includes a "Pure Droop" model for academic comparison.
    ...
    % Inner loop gains for all controllers
    P.kp_v = 0.5; P.ki_v = 150; % Voltage loop gains
    P.kp_i = 10; P.ki_i = 200;  % Current loop gains
    
    % Parameters for VSM-based models
    P.J_vsm = 0.2; P.D_vsm = 5.0; 
    % P.J_vsm = 5.0;  P.D_vsm = 1.5;
    P.T_filter = 0.05; 
    
    % Parameters for Pure Droop model
    P.mp_droop = (0.05 * P.w_nom) / P.P_ref; % 5% frequency droop at rated power
    
    % Parameters for VOC behavioral model
    P.T_voc = 0.02;
    ...
    if strcmp(control_mode, 'Droop')
        % State vector for Pure Droop does not include w_dev
        % x = [iL_a, iL_b, iL_c, vC_a, vC_b, vC_c, delta, p_filt, ...
        %      err_v_d_int, err_v_q_int, err_i_d_int, err_i_q_int]
        x0 = zeros(12, 1);
    else
        % State vector for other models
...
function dxdt = harmonic_dynamics_abc(t, x, P, control_mode)
    ...
    if strcmp(control_mode, 'Droop')
        ...
        w_dev = -P.mp_droop * (p_filt - P.P_ref); % Frequency is algebraic
    else
        iL_abc = x(1:3); vC_abc = x(4:6);
    ...
    switch control_mode
        case 'Droop'
            ...
            dw_dev_dt = []; % Not a state for pure droop
    ...
    % --- Inner Cascaded Voltage and Current Control (Same for Droop/VSM) ---
    ...
    else % VOC has a simpler inner loop in this behavioral model
