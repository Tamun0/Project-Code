...
figure('Name', 'Frequency Response Comparison', 'Position', [100, 100, 1200, 600]);
for g = 1:length(Grid)
    subplot(1, 2, g);
    hold on;
    grid on;
    ...
    for c = 1:length(Controllers)
        ...
        if (g == 1 && is_inductive_controller) || (g == 2 && is_resistive_controller)
            plot(t, results(g,c).w / (2*pi), 'LineWidth', 2, ...
                'LineStyle', line_styles{c}, 'Color', colors{c});
        end
    end
    
    title(['Frequency Response in ', Grid(g).name]);
    xlabel('Time (s)');
    ylabel('Frequency (Hz)');
    ylim([P.f_nom - 1, P.f_nom + 0.5]);
    xline(t_load_step, 'k--', 'Load Step');
    
    % Add annotations based on expected outcomes
    if g == 1 % Inductive Grid
        legend('Droop (Conventional)', 'VSM', 'VOC (Inductive)', 'Location', 'best');
        annotation('textbox', [0.25, 0.2, 0.1, 0.1], 'String', 'VSM and VOC show best performance', 'FitBoxToText','on');
    else % Resistive Grid
        legend('Droop (Inverse)', 'VOC (Resistive)', 'Location', 'best');
        annotation('textbox', [0.7, 0.2, 0.1, 0.1], 'String', 'VOC shows fastest response', 'FitBoxToText','on');
    end
end

% --- Voltage Response Plot ---
figure('Name', 'Voltage Response Comparison', 'Position', [100, 800, 1200, 600]);
for g = 1:length(Grid)
    subplot(1, 2, g);
    hold on;
    grid on;
    ...
    for c = 1:length(Controllers)
       % Logic to plot only appropriate controllers for each grid type
        ...
        if (g == 1 && is_inductive_controller) || (g == 2 && is_resistive_controller)
            plot(t, results(g,c).V, 'LineWidth', 2, ...
                'LineStyle', line_styles{c}, 'Color', colors{c});
        end
    end
    
    title(['Voltage Response in ', Grid(g).name]);
    xlabel('Time (s)');
    ylabel('Voltage (V)');
