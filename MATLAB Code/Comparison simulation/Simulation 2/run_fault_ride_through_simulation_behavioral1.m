function run_fault_ride_through_simulation_behavioral1()
    % Main function to run a comparative simulation of fault ride-through
    % performance for Droop, VSM, and a simplified behavioral VOC.
    ...
    P.J_vsm = 0.0;  P.D_vsm = 50.0; P.T_filter_vsm = 0.02;
    ...
    % ========================================================================
    % State vector for all models: x = [delta, w_dev, p_filt]
    ...
    subplot(3,1,1);
    plot(t, P.f_nom + w_dev / (2*pi), 'b', 'LineWidth', 2);
    hold on;
...
function dxdt = fault_dynamics(t, x, P, control_mode, fault_status)
    ...
    switch control_mode
        ...
        case 'VOC'
            % Simplified behavioral model for VOC
            % Power filter tracks the reference very quickly
            dp_filt_dt = (1/P.T_voc) * (P.P_ref - p_filt);
            
            % Angle controller drives the angle to the target value needed to produce the filtered power
            % This models the fast, self-synchronizing nature of the oscillator
            ...
            % A simple, fast Proportional-Derivative (PD) controller for the angle
