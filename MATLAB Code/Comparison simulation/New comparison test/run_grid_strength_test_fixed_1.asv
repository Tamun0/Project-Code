function run_grid_strength_test_fixed_1()
    % Main function to test the robustness of Droop, VSM, and VOC
    % controllers against varying grid strength (impedance).
    % This version uses separate dynamics functions for each controller
    % to ensure numerical stability and clarity.
    ...
    for c = 1:length(controllers)
        ...
        for z = 1:length(grid_impedances)
            ...
            switch control_mode
                case 'Droop'
                    dynamics_fun = @(t,x) droop_dynamics_for_test(t, x, P);
                    x0 = [0; 0]; % States: [delta, p_filt]
                case 'VSM'
                    dynamics_fun = @(t,x) vsm_dynamics_for_test(t, x, P);
                    x0 = [0; 0; 0]; % States: [delta, w_dev, p_filt]
                case 'VOC'
                    dynamics_fun = @(t,x) voc_dynamics_for_test(t, x, P);
                    x0 = [P.V_peak; 0; 0; 0]; % States: [v_alpha, v_beta, id, iq]
            end

            options = odeset('RelTol', 1e-4, 'AbsTol', 1e-5);
            [t, x] = ode23t(dynamics_fun, t_span, x0, options);
...
% ============================================================================
% DEDICATED DYNAMICS FUNCTIONS
% ============================================================================

function dxdt = droop_dynamics_for_test(t, x, P)
    % Unpack Droop states: [delta, p_filt]
    delta = x(1);
    p_filt = x(2);
...
function dxdt = vsm_dynamics_for_test(t, x, P)
    % Unpack VSM states: [delta, w_dev, p_filt]
    delta = x(1);
    w_dev = x(2);
    p_filt = x(3);
...
function dxdt = voc_dynamics_for_test(t, x, P)
    % Unpack VOC states: [v_alpha, v_beta, id, iq]
    v_alpha = x(1); v_beta = x(2); id = x(3); iq = x(4);
    
    P_ref = P.P_load;
    
    % VOC Dynamics (AHO model)
