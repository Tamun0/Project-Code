function run_inverter_simulation_v7_with_power()
    % Main function to run a final, robust power system simulation.
    % This version includes a plot for the inverter's power output.
    % It uses a slower power ramp and a small virtual inductance
    ...
    P.L_virtual = 1e-3; % A small virtual inductance (1 mH)
    ...
    P_out = zeros(length(t), 1); % Array to store active power
    Q_out = zeros(length(t), 1); % Array to store reactive power
    ...
    for k = 1:length(t)
       ...
       % Calculate instantaneous P and Q at the terminal
       P_out(k) = v_d_terminal * id(k) + v_q_terminal * iq(k);
       Q_out(k) = v_q_terminal * id(k) - v_d_terminal * iq(k);
    end
    ...
    % --- Power Plot (NEW) ---
    subplot(3,1,3);
    plot(t, P_out, 'LineWidth', 2);
    hold on;
    plot(t, Q_out, 'LineWidth', 2);
    title('Inverter Power Output');
    xlabel('Time (s)'); ylabel('Power (W / VAR)');
    legend('Active Power (P)', 'Reactive Power (Q)'); grid on;
