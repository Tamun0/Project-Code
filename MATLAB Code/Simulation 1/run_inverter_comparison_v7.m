function run_inverter_comparison_v7()
    ...
    % ========================================================================
    % 3. POST-PROCESS AND PLOT COMPARATIVE RESULTS
    % ========================================================================
    disp('Simulations finished. Processing results...');
    
    figure('Name', ['Comparative Waveforms for ' grid_type ' Grid'], 'Position', [100 100 900 700]);
    colors = {'b', 'r', 'g'};
    line_styles = {'-', '--', ':'};
    
    % --- Terminal Voltage (ABC Waveforms) Plot ---
    subplot(2,1,1);
    hold on;
    for i = 1:length(results)
        v_abc = post_process_waveforms(results{i}.t, results{i}.x, P, results{i}.name);
        plot(results{i}.t, v_abc(:,1), 'Color', colors{i}, 'LineStyle', line_styles{i}, 'LineWidth', 1.5, 'DisplayName', [results{i}.name ' Va']);
    end
    title(['Three-Phase Terminal Voltage (' grid_type ' Grid)']);
    xlabel('Time (s)'); ylabel('Voltage (V)');
    legend('Location', 'northeast'); grid on; ylim([-400 400]);

    % --- Output Current (ABC Waveforms) Plot ---
    subplot(2,1,2);
    hold on;
    for i = 1:length(results)
        [~, i_abc] = post_process_waveforms(results{i}.t, results{i}.x, P, results{i}.name);
        plot(results{i}.t, i_abc(:,1), 'Color', colors{i}, 'LineStyle', line_styles{i}, 'LineWidth', 1.5, 'DisplayName', [results{i}.name ' Ia']);
    end
    title('Three-Phase Output Current');
...
function [v_abc_terminal, i_abc] = post_process_waveforms(t, x, P, mode)
    ...
    if strcmp(mode, 'VOC')
        ...
        p_filt = x(:,3);
        I_peak_approx = p_filt / P.V_peak * 2; % P = 3/2*V*I -> I = 2/3 * P/V
        V_peak_approx = P.V_peak - I_peak_approx * P.R_grid; % Approximate sag
...
function dxdt = system_dynamics_dq_comparative(t, x, P, mode)
    ...
    L_total = P.L_filt + P.L_grid; R_total = P.R_filt + P.R_grid;
    ...
    ddelta_dt = w_dev;
    v_d_term = v_d_grid + R_total * id - w_gfm * L_total * iq;
    v_q_term = v_q_grid + R_total * iq + w_gfm * L_total * id;
